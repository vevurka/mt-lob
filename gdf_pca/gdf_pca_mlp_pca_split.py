...
def main(stock, r=0.1, s=0.1):
    result_dir = 'res_mlp_pca_gdf_que_prev10_split'
    data_length = 10000
    ...
    for alpha in [0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 2]:
        for hidden_layer_size in [(8, 16), (16, 16), (16, 8), (8, 8)]:
            ...
            scores = svm_gdf_res.train_clf_with_split_pca(clf, feature_name='pca_gdf_que_prev_split10', method='mlp')
            results.append({**scores, 'alpha': alpha, 'solver': solver, 'hidden_layer_sizes': hidden_layer_size})
    pd.DataFrame(results).to_csv(
        os.path.join(result_dir, 'mlp_pca_gdf_split_{}_len{}_r{}_s{}.csv'.format(stock, data_length, r, s)))

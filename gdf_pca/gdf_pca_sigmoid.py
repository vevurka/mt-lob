    ...
    for C in [0.01, 1.0, 100.0]:
        for g in [0.01, 1.0, 100.0]:
            for coef0 in [0.01, 1.0, 100.0]:
                scores = svm_gdf_res.train_svm(C=C, gamma=g, coef0=coef0, kernel='sigmoid', feature_name='pca_gdf_20-30_que3')
                results.append(scores)
    pd.DataFrame(results).to_csv(
        os.path.join(result_dir, 'svm_pca_gdf_sigmoid_{}_len{}_r{}_s{}_K20-30.csv'.format(stock, data_length, r, s)))

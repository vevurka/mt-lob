...
def main(stock, rr, ss):
    results = []
    data_length = 10000
    results_dir = 'res_pca_gdf_que3_10000'
    result_csv = os.path.join(results_dir, 'log_pca_gdf_{}_len{}.csv'.format(stock, data_length))
        ...
        for s in ss:
            try:
                svm_gdf_res = SvmGdfResults(
                    ...
                    gdf_filename_pattern='gdf_{}_len10000_r{}_s{}_K50')
                if not os.path.exists(result_csv):

                    scores = svm_gdf_res.train_svm(clf=LogisticRegression(),
                                                   feature_name='pca_gdf_que3')
                    results.append({**scores, 'r': r, 's': s})
...
if __name__ == '__main__':
    ...
    res = [pool.apply_async(main, [s, [0.1, 1.0], [0.1, 1.0]]) for s in stocks]

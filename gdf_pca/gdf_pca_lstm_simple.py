...
def train_lstm_specific_model(stock, r, s, data_length):
    import tensorflow as tf
    auc_roc = as_keras_metric(tf.metrics.auc)
    r = float(r)
    s = float(s)
    ...
    filename = os.path.join('res_lstm', f'res_lstm_pca_n_{stock}_len{data_length}_r{r}_s{s}.csv')
    # if os.path.exists(filename):
    #     logger.info('Exists %s', filename)
    #     return
    partial_filename = filename + '_partial'
    ...
    for model_name, model_to_run in get_specific_model_func(input_shape=(n_steps, pca.n_components_)).items():
        if np.any(df_partial):
            ...
            if np.any(row):
                ...
                continue
        print(f'Will train {stock} r{r} s{s} {model_name}')
        plot_name = f'plot_lstm/{stock}_func_{model_to_run.__name__}_pca_n_r{r}_s{s}'
        score = gdf_dfs.train_lstm(

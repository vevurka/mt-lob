...
res_dir = 'res_gdf_pca_mlp_iter'


def get_mlp_model_for_arch(arch):
    def get_model():
...
def train_mlp(res):
    ...
    filename = os.path.join(res_dir, f'res_mlp_iter_{stock}_len{data_length}_r{r}_s{s}.csv')
    ...
    if len(df_partial) < 30:
        ...
        get_model = get_mlp_model_for_arch(arch)
        try:
            score = gdf_dfs.train_mlp(
                ...
                compile_kwargs={'loss': 'binary_crossentropy', 'optimizer': 'adam',
                                'metrics': [matthews_correlation, auc_roc]}, class_weight=weights)
            score = {**score, 'r': r, 's': s, 'arch': arch, 'epochs': epochs, 'batch_size': batch_size}
...
def main():
    ...
    train_mlp(df_best[df_best['stock'] == int(s)])

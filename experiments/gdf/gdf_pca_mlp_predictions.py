...
def train_mlp(res):
    ...
    filename = os.path.join(res_dir, f'pred_mlp_iter_{stock}_len{data_length}_r{r}_s{s}.csv')
    if os.path.exists(filename):
        print(f'Exists {filename}.')
        return None

    get_model_func = get_model(arch)
    s, m = gdf_dfs.train_mlp(
...
def main():
    ...
    res = [pool.apply_async(train_mlp, [df_best[df_best['stock'] == int(s)]]) for s in stocks]

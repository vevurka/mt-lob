...
def main(stock):
    results_dir = 'res_prev_que_log'
    data_length = 24000
    log_classify = LobClassifySimple(stock, data_length=data_length, data_dir='../..//data/prepared')

    clf = LogisticRegression(class_weight='balanced')
    scores = log_classify.train_clf(clf, feature_name='que_prev', should_validate=True)
    pd.DataFrame([scores]).to_csv(
       os.path.join(results_dir, 'prev_que_log_{}_len{}.csv'.format(stock, data_length)))
    return {stock: True}
